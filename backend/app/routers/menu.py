...
@router.get("/api/stores/{store_id}/menus", response_model=list[MenuResponse])
async def get_menus(store_id: UUID, category_id: Optional[UUID] = None, svc: MenuService = Depends(get_menu_service)):
    return await svc.get_menus(store_id, category_id)


@router.post("/api/admin/stores/{store_id}/menus", response_model=MenuResponse)
async def create_menu(
    store_id: UUID, name: str = Form(), price: int = Form(), category_id: UUID = Form(),
...
@router.put("/api/admin/stores/{store_id}/menus/{menu_id}", response_model=MenuResponse)
